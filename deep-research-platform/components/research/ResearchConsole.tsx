...
export function ResearchConsole() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      log("search", "Generating research plan..."); await streamSse(`/api/research/${sid}/plan`, (p) => { if ("error" in p && p.error) throw new Error(p.error as string); });
      log("search", "Starting source retrieval..."); await streamSse(`/api/research/${sid}/search`, (p) => { if (p.type === "log") log("search", p.message); if (p.type === "source") { setSources((prev) => [...prev, p.source as CandidateSource]); setCounts(p.counts); } if (p.type === "error") throw new Error(p.error); });
      log("synthesis", "Auto-ranking sources..."); await streamSse(`/api/research/${sid}/rank`, (p) => { if (p.type === "done") setSources(p.ranked as CandidateSource[]); if (p.type === "error") throw new Error(p.error); });
      log("synthesis", "Verifying citations..."); await streamSse(`/api/research/${sid}/verify`, (p) => { if (p.type === "verified") setCitations((prev) => [...prev, p.item as VerifiedCitation]); if (p.type === "error") throw new Error(p.error); });
